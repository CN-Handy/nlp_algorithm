...
# 读取数据集
questions = json.load(open("questions.json")) # 历史用户提问
pdf = pdfplumber.open("汽车知识手册.pdf") # 知识库
pdf_content = []
for page_idx in range(len(pdf.pages)): # 按照页面内容提取
    pdf_content.append({
...
question_feat = normalize(question_feat) # 归一化，最终相似度打分在规整范围中

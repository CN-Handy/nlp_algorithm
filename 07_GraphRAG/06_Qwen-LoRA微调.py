# lora 比较适合 大模型 或 aigc大模型，也可以用在bert上，但是一般不这样做。
# lora 可以微调新增加参数，参数 是原始模型参数的1%的比例，lora微调后 可以达到 full finetune的精度。

import pandas as pd
from datasets import Dataset
from transformers import (
    AutoTokenizer,
    AutoModelForCausalLM, # AutoModelForCausalLM 加载LLM
...
def load_and_preprocess_data():
    """加载和预处理数据"""
    train_data = pd.read_csv('intent-dataset.csv', sep='\t', header=None) # 加载比赛的数据集 + 外部数据集（同类型的数据集、大模型）
...
def initialize_model_and_tokenizer(model_path):
    ...
    # 加载模型（GPT文本生成，基于前面的句子生成后面的句子）
    model = AutoModelForCausalLM.from_pretrained(  # gemma、qwen、ds
        model_path,
        device_map="auto",
...
def setup_lora(model):
    ...
    model = get_peft_model(model, config) # 原始qwen模型 -》  高效微调的模型
